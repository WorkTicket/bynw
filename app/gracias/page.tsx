...
import GraciasCrossSell from "@/components/GraciasCrossSell"
 ...
 * Point Hotmart thank-you here (see .env.example).
 * Optional query: transaction, value, currency, content_name, product (slug).
 */
export default function GraciasPage({ searchParams }: Props) {
  const excludeSlug =
    firstParam(searchParams?.product) ||
    firstParam(searchParams?.slug) ||
    firstParam(searchParams?.content_ids) ||
    null
  ...
  return (
    <>
      ...
      <GraciasCrossSell excludeSlug={excludeSlug} />
    </>
  )
}
